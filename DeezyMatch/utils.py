...
# ------------------- sort_key_trans --------------------
def sort_key_trans(item):
    item = os.path.abspath(item)
    item2match = os.path.basename(item)
    file_name = item2match[0:item2match.rfind('_')]
    file_number = int(item2match[item2match.rfind('_')+1:])
...
def string_split(x, tokenize=["char"], min_gram=1, max_gram=3, token_sep="default", prefix_suffix=["|", "|"]):
    ...
    if "ngram" in tokenize:
        ...
        for ngram in range(min_gram, max_gram+1):
            tokenized_str = tokenized_str + [x_bounded[i:i+ngram] for i in range(len(x_bounded)-ngram+1)] 
