...
class DatasetClass(Dataset):
    def __init__(self, dataset_split, dataset_vocab, maxlen=256):
        self.maxlen = maxlen
...
def createStringMatchingTest(dataset_path, output_test_path, csv_sep="\t"):
    with open(dataset_path, "r", encoding="utf8") as ds_fio:
        df_list = ds_fio.readlines()
        for i in range(len(df_list)):
            tmp_split_row = df_list[i].split(csv_sep)
...
def m_split_dataset(dataset_path, train_prop=0.7, val_prop=0.15, test_prop=0.15, csv_sep="\t"):
    ...
    with open(dataset_path, "r", encoding="utf8") as ds_fio:
        ...
        for i in range(len(df_list)):
            ...
            df_list[i] = tmp_split_row[:3]
    dataset_pd = pd.DataFrame(df_list, columns=["s1", "s2", "label"])
    #dataset_pd["t4"] = dataset_pd["t4"].str.strip()

    n_total = len(dataset_pd)
    ...
    train = dataset_pd[:n_train]
    valid = dataset_pd[n_train:n_train+n_val]
    test = dataset_pd[n_train+n_val:n_train+n_val+n_test]
