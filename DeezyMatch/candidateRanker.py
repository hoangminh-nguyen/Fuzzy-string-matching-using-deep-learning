...
from .rnn_networks import inference
...
# ------------------- candidate_ranker - TRANSFORMER--------------------
def candidate_ranker_trans(model_name, input_file_path="default", query_scenario=None, candidate_scenario=None,
                     ranking_metric="faiss", selection_threshold=0.8, 
                     query=None, num_candidates=10, search_size=4, output_path="ranker_output",
                     pretrained_model_path=None, pretrained_vocab_path=None, number_test_rows=-1):
    """
    ...
    # ----- CANDIDATES
    file_pt = model_name + ".pt"
    file_id = model_name + "_id.pt"
    file_item = model_name + "_items.npy"

    path_combined = os.path.join(candidate_scenario, file_pt)
    path_id_combined = os.path.join(candidate_scenario, file_id)
    path_items_combined = os.path.join(candidate_scenario, file_item)
    ...
    vecs_candidates = torch.load(path_combined, map_location=dl_inputs['general']['device'])
    ...
    if query:
        ...
        np.save(os.path.join(tmp_dirname, file_item), vecs_items)
        path_items_combined = os.path.join(tmp_dirname, file_item)
    else:
        path_items_combined = os.path.join(query_scenario, file_item)

    path_combined = os.path.join(query_scenario, file_pt)
    path_id_combined = os.path.join(query_scenario, file_id)
    ...
    vecs_query = torch.load(path_combined, map_location=dl_inputs['general']['device'])
