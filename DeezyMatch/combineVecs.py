...
from .utils import sort_key, sort_key_trans
...
# ------------------- combine_vecs - Transformer --------------------
def combine_vecs_trans(input_file_path="default", model_name="tranformer_cn",
                 input_scenario="test",  
                 output_scenario="test", print_every=500, 
                 sel_device="default", save_df=True):
    """
    ...
    path_vec_combined = os.path.join(output_scenario, f"{model_name}.pt")
    path_id_combined = os.path.join(output_scenario, f"{model_name}_id.pt")
    path_items_combined = os.path.join(output_scenario, f"{model_name}_items.npy")
    inp_par_dir = os.path.join(input_scenario)
    ...
    list_files.sort(key=sort_key_trans)
    vecs = []
    for i, lfile in enumerate(list_files):
        if i % print_every == 0: print("%07i" % i, lfile)
        print(i)
...
def main():
    ...
    combine_vecs_trans(input_file_path=input_file_path, 
                 input_scenario=input_scenario, 
                 output_scenario=output_scenario)
